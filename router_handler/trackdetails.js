...
exports.tenPublicList = (req, res) => {
    ...
    const sql = `select play_list.user_name,play_list.list_id,list_name,count(trackInList.list_id) as count ,sum(TIME_TO_SEC(track_duration))as total_time from ((raw_tracks join trackInList on trackInList.track_id=raw_tracks.track_id ) join play_list on play_list.list_id = trackInList.list_id and play_list.public = 1)   group by list_name ORDER BY play_list.update_time limit 10 `    
...
//select play_list.user_name,list_name,count(trackInList.list_id) as count ,sum(TIME_TO_SEC(track_duration))as total_time from 
        ...
        const sql = `select play_list.user_name,list_name,play_list.list_id,play_list.public,play_list.description, count(trackInList.list_id) as count ,sum(TIME_TO_SEC(track_duration))as total_time from ((raw_tracks join trackInList on trackInList.track_id=raw_tracks.track_id ) join play_list on play_list.list_id = trackInList.list_id  and play_list.user_name = '${name}')   group by list_name ORDER BY play_list.update_time limit 20 `    
...
exports.newpolicy = (req, res) => {
    ...
    const sql = `select policy_name from Policy where policy_name='${policy_name}'`
    database.query(sql, (err, results) => {
            if (err) return res.send({ status: 401, message: err.message })
            if(results.length!==0) return res.send({status:401,message:"This Policy already exists, please update it!"})
